...
//                       <div key={cellIndex} className={`bingo-cell ${cell.is_completed ? 'completed' : ''}`} style={{width: '30vw'}}>
//                         {cell.content}
//                       </div>
//                     ))}
//                   </div>
            ...
            bingoBoards.map((board, index) => (
              <div key={index} className="start-bingo" onClick={() => handleBingoClick(board.bingo_id, board.team_id)}>
                ...
                <div className="start-bingo-grid">
                  {board.bingo_cells.reduce((acc, cell, i) => {
                    if (i % 3 === 0) acc.push([]);
                    acc[acc.length - 1].push(cell);
                    return acc;
                  }, []).map((row, rowIndex) => (
                    <div key={rowIndex} className="bingo-row" style={{ display: 'flex', justifyContent: 'center' }}>
                      {row.map((cell, cellIndex) => (
                        <div key={cellIndex} className={`start-bingo-cell ${cell.is_completed ? 'completed' : ''}`} style={{ width: '30vw' }}>
                          {cell.content}
                        </div>
                      ))}
                    </div>
